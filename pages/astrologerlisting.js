...
function AstrologerListing() {
  ...
  function filterHandler(state) {
    initializePaginationData(
      astrologersList.filter((e) => {
        if (state[e?.currentStatus]) return true;
        else return false;
      })
    );
  }
    ...
    <div className={styles.astrologerlist}>
      ...
      <div>
        ...
        <div className={styles.cardscontainer}>
          <div className={styles.title} data-status="green">
            Astrologers online - 20
          </div>
          <div className={styles.cards}>
            {paginationData.slice(firstItemNum, lastItemNum).map((e) => (
              <Card key={e.id} cssmodule={styles} data={e} />
            ))}
          </div>
        </div>
      </div>
