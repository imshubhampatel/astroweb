  ...
  return (
    <div className={`${styles.container}  d-flex gap-2 `}>
      ...
      <div className={`flex-grow-1  `} style={{ height: "100%" }}>
        ...
        <div className={`${styles.orderDetailText}`}>
          Order id: {data?.id} <br />
          {data?.scheduledTime.toDate().toDateString() + " "}{data?.scheduledTime ? new Date(data?.scheduledTime.seconds*1000).toLocaleTimeString([], {timeStyle: 'short'}): null}
        </div>
      ...
      <div className={` ps-2 pe-2`}>
        ...
        <div className={`row  ${styles.orderDetailText}`}>
          ...
          <div className={`col text-end`}>{ data?.totalDuration - (data?.lastDuration - data?.lastActualDuration ) >= 0 ? data?.totalDuration - (data?.lastDuration - data?.lastActualDuration ) : 0 } minutes</div>
        </div>
